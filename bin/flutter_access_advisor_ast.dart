...
import 'package:flutter_access_advisor_cli/src/ast_report_generator.dart';
...
import 'package:flutter_access_advisor_cli/src/usage_command.dart' show UsageCommand;
...
class VersionInfo {
  ...
  factory VersionInfo.fromYaml(String yamlString) {
    final yaml = loadYaml(yamlString) as YamlMap;
    return VersionInfo(
      version: yaml['version'] as String,
      buildNumber: yaml['build_number'] as int,
      date: yaml['date'] as String,
      changelog: List<String>.from(yaml['changelog'] as List),
    );
  }

  @override
  String toString() => 'v$version (build $buildNumber, $date)';
}
...
VersionInfo loadVersionInfo() {
  try {
    ...
    final versionFile = File('$projectRoot/version.yaml');
    
    if (!versionFile.existsSync()) {
      throw Exception('version.yaml not found at ${versionFile.absolute.path}');
    }
    final yamlString = versionFile.readAsStringSync();
    return VersionInfo.fromYaml(yamlString);
  } catch (e) {
    stderr.writeln('Warning: Could not load version information: $e');
    return VersionInfo(
      version: '0.0.0',
      buildNumber: 0,
      date: 'unknown',
      changelog: ['Could not load version information'],
    );
  }
}
...
Future<void> _handleAnalysisCommand(List<String> arguments) async {
  ...
  try {
    ...
    if (results['version']) {
      final versionInfo = loadVersionInfo();
      print('Conalyz CLI ${versionInfo.version}');
      print('Build: ${versionInfo.buildNumber} (${versionInfo.date})');
      print('\nRecent changes:');
      for (final change in versionInfo.changelog) {
        print('• $change');
      }
      return;
    }
