    ...
    if (widget.type == 'MergeSemantics') {
      ...
      if (!_hasMultipleSemanticChildren(widget)) {
        issues.add(AccessibilityIssue(
          ...
          rule: ruleId,
          suggestion: 'Use MergeSemantics only when grouping multiple widgets with semantic information',
        ));
      }
    }
...
class TapTargetSizeRule extends AccessibilityRule {
  ...
  @override
  List<String> get targetWidgets => ['GestureDetector', 'InkWell', 'IconButton', 'ElevatedButton', 'TextButton'];

  @override
  List<AccessibilityIssue> check(WidgetInfo widget, String filePath, PlatformType platform) {
    final issues = <AccessibilityIssue>[];
    
    if (['GestureDetector', 'InkWell', 'IconButton', 'ElevatedButton', 'TextButton'].contains(widget.type)) {
      if (!_hasAdequateTapTargetSize(widget)) {
        issues.add(AccessibilityIssue(
          ...
          column: widget.column,
          rule: ruleId,
          suggestion: 'Ensure interactive targets are at least 48x48dp. Use padding, constraints, or SizedBox to increase touch area.',
        ));
      }
    }
...
class AnimationControlRule extends AccessibilityRule {
  ...
  @override
  String get description => 'Animations should respect reduced motion preferences';

  @override
  List<String> get targetWidgets => ['AnimatedContainer', 'AnimatedOpacity', 'AnimationController'];

  @override
  List<AccessibilityIssue> check(WidgetInfo widget, String filePath, PlatformType platform) {
    final issues = <AccessibilityIssue>[];
    
    if (['AnimatedContainer', 'AnimatedOpacity', 'AnimationController'].contains(widget.type)) {
      if (!_respectsReducedMotion(widget)) {
        issues.add(AccessibilityIssue(
          ...
          column: widget.column,
          rule: ruleId,
          suggestion: 'Check MediaQuery.of(context).disableAnimations or provide animation toggle',
        ));
      }
    }
...
class CustomErrorAnnouncementRule extends AccessibilityRule {
  ...
  @override
  String get description => 'Custom errors should be announced to assistive technology';
  ...
  List<AccessibilityIssue> check(WidgetInfo widget, String filePath, PlatformType platform) {
    ...
    if (_hasCustomErrorHandling(widget) && !_announcesErrors(widget)) {
      issues.add(AccessibilityIssue(
        ...
        column: widget.column,
        rule: ruleId,
        suggestion: 'Use SemanticsService.announce() or Semantics liveRegion for error announcements',
      ));
    }
  ...
  bool _hasCustomErrorHandling(WidgetInfo widget) {
    ...
    return code.contains('error') && 
           (code.contains('setState') || code.contains('showDialog') || code.contains('SnackBar'));
  }
    ...
    if (widget.type == 'Semantics' && _isHeader(widget)) {
      if (!_hasSortKey(widget)) {
        issues.add(AccessibilityIssue(
          ...
          column: widget.column,
          rule: ruleId,
          suggestion: 'Add sortKey property to define heading hierarchy (e.g., sortKey: OrdinalSortKey(1.0))',
        ));
      }
    }
    ...
    if (widget.type == 'Table') {
      if (!_hasProperTableStructure(widget)) {
        issues.add(AccessibilityIssue(
          ...
          column: widget.column,
          rule: ruleId,
          suggestion: 'Use TableRow and TableCell widgets to structure table content properly',
        ));
      }
    }
    ...
    if (widget.type == 'FocusTraversalOrder') {
      if (!_hasExplicitOrder(widget)) {
        issues.add(AccessibilityIssue(
          ...
          column: widget.column,
          rule: ruleId,
          suggestion: 'Add order property to define explicit focus traversal order (e.g., order: NumericFocusOrder(1.0))',
        ));
      }
    }
...
class LiveRegionRule extends AccessibilityRule {
  ...
  @override
  String get description => 'Dynamic content should be announced to assistive technology';
    ...
    if (_hasDynamicContent(widget)) {
      ...
      if (!_hasLiveRegion(widget)) {
        issues.add(AccessibilityIssue(
          ...
          column: widget.column,
          rule: ruleId,
          suggestion: 'Wrap dynamic content with Semantics(liveRegion: true) or use SemanticsService.announce()',
        ));
      }
    }
...
class SemanticTraversalOrderRule extends AccessibilityRule {
  ...
  @override
  String get description => 'Semantic traversal order should use OrdinalSortKey';
  ...
  List<AccessibilityIssue> check(WidgetInfo widget, String filePath, PlatformType platform) {
    ...
    if (widget.type == 'Semantics' && _hasSortKey(widget) && !_hasOrdinalSortKey(widget)) {
      issues.add(AccessibilityIssue(
        ...
        column: widget.column,
        rule: ruleId,
        suggestion: 'Use OrdinalSortKey instead of custom sort keys for better accessibility support',
      ));
    }
    ...
    if (widget.type == 'Scaffold') {
      if (!_hasNavigationElements(widget)) {
        issues.add(AccessibilityIssue(
          ...
          column: widget.column,
          rule: ruleId,
          suggestion: 'Add AppBar, BottomNavigationBar, or Drawer for clear navigation structure',
        ));
      }
    }
...
class TextScalingSupportRule extends AccessibilityRule {
  ...
  @override
  String get description => 'Text should scale properly with system font size settings';
  ...
  List<AccessibilityIssue> check(WidgetInfo widget, String filePath, PlatformType platform) {
    ...
    if (widget.type == 'Text') {
      ...
      if ((hasHardcodedSize && !isProperlyScaled && !isWrappedInMediaQuery) || hasFixedStyle) {
      ...
      if (platform == PlatformType.mobile && 
        ...
        issues.add(AccessibilityIssue(
          ...
          column: widget.column,
          rule: ruleId,
          suggestion: 'Use minimum font size of 12sp or theme-based text styles that scale appropriately',
        ));
      }
    }
