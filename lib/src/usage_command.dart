...
class UsageCommand {
  ...
  Future<void> showBasicUsage() async {
    try {
      ...
      print('');
      print(
          '📈 Total lines scanned: ${_formatNumber(statistics.totalLinesScanned)}');
      print('🔄 Analysis sessions: ${statistics.totalSessions}');
      print(
          '📅 Date range: ${_formatDateRange(statistics.firstUsage, statistics.lastUsage)}');
      print(
          '📊 Average lines per session: ${_formatNumber(statistics.averageLinesPerSession.round())}');
      print(
          '📁 Total files analyzed: ${_formatNumber(statistics.totalFilesAnalyzed)}');
      print(
          '⏱️  Average analysis time: ${_formatDuration(statistics.averageAnalysisTimeMs.round())}');
      print('');
  ...
  Future<void> showDetailedUsage() async {
    try {
      ...
      if (statistics.totalSessions == 0) {
        print('📊 Conalyz - Detailed Usage Statistics');
        print('');
        print('No analysis sessions recorded yet.');
        print('Run an analysis to start tracking your usage.');
        print('');
      ...
      if (platformStats.isNotEmpty) {
        ...
        for (final entry in platformStats.entries) {
          final percentage =
              (entry.value / statistics.totalLinesScanned * 100).round();
          final progressBar = _createProgressBar(percentage, 20);
          print(
              '   ${entry.key.padRight(8)}: ${_formatNumber(entry.value).padLeft(8)} lines (${percentage.toString().padLeft(2)}%) $progressBar');
        }
      }
      ...
      if (largestSession != null) {
        ...
        print(
            '   ${_formatNumber(largestSession.linesScanned)} lines on ${_formatDateTime(largestSession.timestamp)}');
      }
      ...
      for (int i = 0; i < statistics.recentSessions.length; i++) {
        ...
        print(
            '${sessionNumber.toString().padLeft(2)}. ${_formatDateTime(session.timestamp)}');
