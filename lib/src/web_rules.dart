...
class WebSemanticHtmlRule extends AccessibilityRule {
  ...
  String get description =>
      'Semantics widgets should have proper HTML tag mapping for web';
    ...
    if (widget.type == 'Semantics') {
      if (!_hasTagName(widget)) {
        issues.add(AccessibilityIssue(
          ...
          rule: ruleId,
          suggestion:
              'Add tagName property to Semantics widget for proper HTML mapping (e.g., tagName: "main", "section", "article")',
        ));
      }
    }
...
class WebAriaLabelsRule extends AccessibilityRule {
  ...
  @override
  String get description =>
      'Interactive elements should have proper ARIA labels for web';
    ...
    if (widget.type == 'Semantics' && _isInteractiveElement(widget)) {
      if (!_hasAriaLabels(widget)) {
        issues.add(AccessibilityIssue(
          ...
          rule: ruleId,
          suggestion:
              'Add label and hint properties to Semantics for proper ARIA labeling',
        ));
      }
    }
  ...
  bool _hasAriaLabels(WidgetInfo widget) {
    return widget.properties.containsKey('label') ||
        ...
        (widget.sourceCode.contains('label:') &&
            widget.sourceCode.contains('hint:'));
  }
}
...
class WebPageTitlesRule extends AccessibilityRule {
  ...
  @override
  String get description =>
      'Apps should have proper titles for web accessibility';
    ...
    if (['MaterialApp', 'CupertinoApp'].contains(widget.type)) {
      if (!_hasTitle(widget)) {
        issues.add(AccessibilityIssue(
          ...
          rule: ruleId,
          suggestion:
              'Add title property to MaterialApp/CupertinoApp for proper page title',
        ));
      }
    }
...
class WebFocusNavigationRule extends AccessibilityRule {
  ...
  @override
  String get description =>
      'Focus elements should have proper management for web';
    ...
    if (['Focus', 'FocusScope'].contains(widget.type)) {
      if (!_hasProperFocusManagement(widget)) {
        issues.add(AccessibilityIssue(
          ...
          rule: ruleId,
          suggestion:
              'Add focusNode or autofocus property for proper focus management',
        ));
      }
    }
